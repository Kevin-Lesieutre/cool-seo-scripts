(function() {
    const styles = `
        :root {
            ...
            --primary-gradient: linear-gradient(135deg, #e0314f 0%, #b8001a 100%);
            --highlight-bg: #fdecea;
            ...
            --warning-poor-color: #f29900;
            --warning-medium-color: #f9ab00;
        ...
        #seo-analysis-output {
             font-family: var(--font-stack); padding: 15px; margin: 30px auto; background-color: transparent;
             border-radius: var(--border-radius); color: var(--text-color); max-width: 1100px; line-height: 1.6;
             background:white;
        }
        ...
        .detail-item { display: flex; align-items: center; margin-bottom: 12px; font-size: 0.9em; color: var(--text-secondary); padding-bottom: 12px; border-bottom: 1px dotted var(--border-color); }
        ...
        .detail-item .detail-value { font-weight: 600; color: var(--text-color); word-break: break-word; text-align: right; flex-grow: 1; }
        ...
        #seo-analysis-output h2 { display: none; }
        #seo-analysis-output h3 { display: none; }
        #seo-analysis-output table { margin-top: 0; box-shadow: var(--card-shadow); border-top-left-radius:0px; border-top-right-radius:0px }
        ...
        @media (max-width: 768px) { .overview-column.ranking-col, .overview-column.details-col { flex-basis: 100%; } }
    ...
    try {
        ...
        console.error("Error during data extraction/processing:", error); const errorMsg = document.createElement('div'); errorMsg.className = 'error-message'; errorMsg.textContent = `An error occurred: ${error.message}. Check console.`; outputContainer.appendChild(errorMsg); document.body.appendChild(outputContainer); return;
    }

    function createTableRow(rowDataForVisibleCells, columnConfig, fullDataObject, isHeader = false, highlight = false) {
    ...
    overviewCard.appendChild(logoImg);
    ...
    if (averageTargetPosition !== null && foundPositionCount > 0 && currentQuery && firstTargetUrlFound) {
        ...
        urlP.innerHTML = `<a href="${firstTargetUrlFound}" target="_blank" rel="noopener noreferrer">${firstTargetUrlFound}</a>`;
        ...
        if (sortedPositions.length > 0) { const distTitle = document.createElement('p'); distTitle.className = 'distribution-title'; distTitle.textContent = 'Position Distribution:'; rankingColumn.appendChild(distTitle); const distList = document.createElement('ul'); distList.className = 'distribution-list'; sortedPositions.forEach(pos => { const count = targetPositionCounts[pos]; const li = document.createElement('li'); li.innerHTML = `Posistion <span class="pos-count">${pos}</span> on <span class="pos-count">${count}</span> Datacenter(s)`; distList.appendChild(li); }); rankingColumn.appendChild(distList); }
         ...
         const queryP = document.createElement('p'); queryP.className = 'ranking-summary'; queryP.innerHTML = `For query: <strong class="query">${currentQuery.replace(/</g, "&lt;")}</strong>`; rankingColumn.appendChild(queryP); const noRankP = document.createElement('p'); noRankP.className = 'no-ranking-info'; if (actualDatacenters > 0) { noRankP.textContent = 'Target URL not found ranking in the results.'; } else { noRankP.textContent = 'No responding datacenters to analyze ranking.'; } rankingColumn.appendChild(noRankP);
    } else {
        const noDataP = document.createElement('p'); noDataP.className = 'no-ranking-info'; noDataP.textContent = 'No ranking data available.'; rankingColumn.appendChild(noDataP);
    }
    ...
    function addDetailItem(iconSvg, label, value, valueClass = '') {
        const item = document.createElement('div'); item.className = 'detail-item'; item.innerHTML = `${iconSvg}<span class="detail-label">${label}:</span><span class="detail-value ${valueClass}">${value}</span>`; detailsContainer.appendChild(item);
    }

    addDetailItem(ICONS.clock, 'Timestamp', new Date().toLocaleString());
    if (currentLangCode) {
        const flag = countryCodeToEmoji(currentLangCode);
        addDetailItem(ICONS.earth, 'Region', `<span class="flag-emoji" aria-label="${currentLangCode.toUpperCase()} flag">${flag}</span> ${currentLangCode.toUpperCase()}`);
    }
